...
class Translator {
    ...
    /**
     * 检测规则原文是否出现在 IPC 发送上下文中
     * 用于防止翻译 sendRequest/postMessage 中的协议字符串
     *
     * 策略：提取所有 IPC 调用中 type/command 的值，检查规则原文是否在其中
     * - 使用 [\s\S]*? 穿过嵌套花括号（旧版 [^}]*? 会在嵌套 } 处停止）
     * - 使用 (['"])...\1 支持单引号和双引号
     * - 贪婪回溯到最后一个 type/command，确保目标值紧跟其后
     */
    isInIpcContext(content, ruleOriginal) {
        // 提取所有 sendRequest/postMessage 中 type/command 的值
        ...
        // 规则原文通常带双引号（如 '"install_plugin"'），提取纯值用于跨引号匹配
        ...
        for (const rule of rules) {
            ...
            try {
                if (rule.regex) {
                    ...
                    if (this.isInIpcContext(content, rule.original)) {
                        console.warn(
