        ...
        try {
            ...
            for (const config of configList) {
                ...
                for (const [key, prop] of Object.entries(config.properties)) {
                    // 翻译 title
                    if (settingsRules.titles?.[key]) {
                        prop.title = settingsRules.titles[key];
                    }
                    // 翻译 description
                    if (prop.description && settingsRules.descriptions?.[key]) {
                        prop.description = settingsRules.descriptions[key];
                    }
                    // 翻译 markdownDescription
                    if (prop.markdownDescription && settingsRules.descriptions?.[key]) {
                        prop.markdownDescription = settingsRules.descriptions[key];
                    }
                    // 翻译 enumDescriptions
                    if (prop.enumDescriptions && settingsRules.enumDescriptions?.[key]) {
                        const translations = settingsRules.enumDescriptions[key];
                        prop.enumDescriptions = prop.enumDescriptions.map((desc, i) =>
                            translations[i] || desc
                        );
                    }
                }
